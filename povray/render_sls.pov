...
#declare datafile = concat("/srv/home/pazouki/chronoStuff/runsChrono/run494/data_slsdata_", str(fnum,0,0), ".txt")
//#declare datafile = concat("C:\\Users\\Jovanny\\Desktop\\School\\Pazouki\\POV-Ray\\Animations\\data\\data_slsdata_", str(fnum,-4,0), ".txt")  
...
//#declare cam_loc    = <0, -0.09, -.00>;
//#declare cam_lookat = <0, 0, -.025>;
#declare cam_loc = <75,0,25>;
#declare cam_lookat = <-0,0,25>;

     
#declare meshLoc = cam_loc;
#declare locIndicator = 1;
// -------------------------------------------------------           
     
#declare light_loc    = <2, 0, 1>;
...
#macro position(L, Q)
   ...
   // #local pQ = <Q.y,Q.t,Q.z,-Q.x>;  // i.e., <e1,e3,e2,-e0>    
        #local pL = <L.x, L.y, L.z>;  
    #local pQ = <Q.y,Q.z,Q.t,Q.x>;  // i.e., <e1,e3,e2,-e0>  
    QMatrix(pQ) 
    translate pL
#end     
...
#macro XYZframe(len, rad)  
   ...
   #local Yellow_texture  = texture {pigment{color rgb<0.8,0.8,0>} finish{phong 1}} 
    ...
    #switch (shape)
        ...
        #case (0)
		    ...
		    sphere {
			    <0,0,0>, ar
				position(<ax,ay,az>,<e0,e1,e2,e3>)
				pigment {color rgbt <2*cR, 2*cG, 2*cB, 0> }
       ...
       // ellipsoid -------------  
        ...
        #case (2)
            ...
            /*
            			...
            			<hx, hy, hz>
            			position(<ax,ay,az>,<e0,e1,e2,e3>)
            			pigment {color rgbt <10*cR, 10*cG, 10*cB, 0>}
            			finish {diffuse 1 ambient 0.0 specular .05}
            		    }  
            	    #else
            	        box {
            			...
            			<hx, hy, hz>
            			position(<ax,ay,az>,<e0,e1,e2,e3>)
            			pigment {color rgbt <9*cR, 9*cG, 0*cB, 0>}
            			finish {diffuse 1 ambient 0.0 specular .05}
            		    }
            		#end
        		#else              		
        			...
        			<hx, hy, hz>
        			position(<ax,ay,az>,<e0,e1,e2,e3>)
        			pigment {color rgbt <4*cR, 4*cG, 8*cB, 0>}
        			finish {diffuse 1 ambient 0.0 specular .05}
        		    } 
        ...
        #case (3)
			...
			#if (render_objects & (active | render_static))
				cylinder {
			        <p1x,p1y,p1z>, <p2x,p2y,p2z>, ar      
					pigment {color rgbt <0.3*cR, 0.3*cG, 0.3*cB, 0> transmit 0}
					position(<ax,ay,az>,<e0,e1,e2,e3>)     
					finish {diffuse 1 ambient 0.0 specular .05 }
				}   
			#end
		...
		#local subMeshName = substr(strMeshName, 1, 3);
          ...
          #if (locIndicator)
            #declare locIndicator = 0;
            #declare meshLoc = <ax,ay,az>;
          #end
			#end
...
//#declare meshLoc = <meshLoc.x, -8, meshLoc.z>;
//#declare cam_loc = meshLoc;
